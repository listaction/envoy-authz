...
public class ZanzibarImpl implements Zanzibar {
  ...
  public CheckResult check(String namespace, String object, String relation, String principal, LocalCache requestCache) {
    ...
    return CheckResult.builder().result(relations.contains(tag)).tags(relations).build();
  }
  ...
  private Set<Tuple2<String, String>> lookup(Set<ExpandedAcl> relations, String namespace, String object, String principal) {
    ...
    for (ExpandedAcl t : filtered) {
      ...
      if (principal.equals(user)) {
        result.add(
            Tuples.of(String.format("%s:%s", t.getNamespace(), t.getObject()), t.getRelation()));
      } else if (Strings.isEmpty(user)) {
        Set<Tuple2<String, String>> nested =
            lookup(relations, t.getUsersetNamespace(), t.getUsersetObject(), principal);
        if (nested.size() > 0) {
          Tuple2<String, String> rightPart =
              Tuples.of(
                  String.format("%s:%s", t.getUsersetNamespace(), t.getUsersetObject()),
                  t.getUsersetRelation());
          if (nested.contains(rightPart)) {
            result.add(
                Tuples.of(
                    String.format("%s:%s", t.getNamespace(), t.getObject()), t.getRelation()));
          }
  ...
  private Set<ExpandedAcl> expandMultiple(Set<Tuple2<String, String>> namespaceObjects, String principal, LocalCache requestCache) {
    ...
    for (Acl acl : acls) { // acls found by user
  ...
  private Set<ExpandedAcl> expand(String namespace, String object, String principal, Set<Acl> acls, LocalCache requestCache) {
    ...
    for (Acl acl : acls) {
      Set<String> nested =
          relationConfigService.nestedRelations(
              acl.getNamespace(), acl.getObject(), acl.getRelation());
      if (acl.hasUserset()) {
        Set<Tuple2<String, String>> aclsToExpand = new HashSet<>();
        Set<String> roots =
            relationConfigService.rootRelations(
                acl.getUsersetNamespace(), acl.getUsersetObject(), acl.getUsersetRelation());
        for (String rel : nested) {
          for (String rootRel : roots) {
            ...
            if (cache.containsKey(Tuples.of(acl.getUsersetNamespace(), acl.getUsersetObject()))) {
              Set<ExpandedAcl> dataFromCache =
                  cache.get(Tuples.of(acl.getUsersetNamespace(), acl.getUsersetObject()));
  ...
  public static class ExpandedAcl {
    ...
    public boolean equals(Object o) {
      ...
      return Objects.equals(namespace, that.namespace)
          ...
          && Objects.equals(usersetNamespace, that.usersetNamespace)
          && Objects.equals(usersetObject, that.usersetObject)
          && Objects.equals(usersetRelation, that.usersetRelation);
    }
    ...
    public String toString() {
      ...
      return String.format(
          "%s:%s#%s@%s:%s#%s",
          namespace, object, relation, usersetNamespace, usersetObject, usersetRelation);
    }
  }
}
