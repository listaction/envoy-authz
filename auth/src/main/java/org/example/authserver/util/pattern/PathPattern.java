...
package org.example.authserver.util.pattern;
 ...
 * </ul>
 *
 * <p><strong>Note:</strong> In contrast to {@link org.springframework.util.AntPathMatcher}, {@code
 * **} is supported only at the end of a pattern. For example {@code /pages/{**}} is valid but
 * {@code /pages/{**}/details} is not. The same applies also to the capturing variant <code>
 * {*spring}</code>. The aim is to eliminate ambiguity when comparing patterns for specificity.
 *
 * <h3>Examples</h3>
 *
 * <ul>
 ...
 *       /resources/}, as well as {@code /resources}, and captures their relative path in a variable
 *       named "path"; {@code /resources/image.png} will match with "path" &rarr; "/image.png", and
 *       {@code /resources/css/spring.css} will match with "path" &rarr; "/css/spring.css"
...
public class PathPattern implements Comparable<PathPattern> {
  ...
  public PathRemainingMatchInfo matchStartOfPath(PathContainer pathContainer) {
    ...
    if (!matches) {
      ...
      return new PathRemainingMatchInfo(
          pathMatched, pathRemaining, matchingContext.getPathMatchResult());
    }
  }
  ...
  public static class PathRemainingMatchInfo {
    ...
    PathRemainingMatchInfo(
        PathContainer pathMatched, PathContainer pathRemaining, PathMatchInfo pathMatchInfo) {
  ...
  public class MatchingContext {
    ...
    @jakarta.annotation.Nullable private Map<String, String> extractedUriVariables;
