...
class ZanzibarImplTest {
  ...
  @Mock private AclRelationConfigRepository configRepository;
  ...
  void setUp() {
    ...
    aclRelationConfigService =
        new AclRelationConfigService(configRepository, cacheService, subscriptionRepository);
  ...
  void check() {
    ...
    for (Map.Entry<String, Boolean> entry : usersToTest.entrySet()) {
      ...
      Set<Acl> aclsDocReadme =
          acls.stream()
              .filter(acl -> acl.getNamespace().equals("doc") && acl.getObject().equals("readme"))
              .collect(Collectors.toSet());
      Set<Acl> aclsGroupDocument =
          acls.stream()
              .filter(
                  acl -> acl.getNamespace().equals("group") && acl.getObject().equals("document"))
              .collect(Collectors.toSet());
      ...
      Mockito.doReturn(aclsDocReadme)
          .when(aclService)
          .findAllByPrincipalAndNsObjectIn(eq(principal), eq(List.of("doc:readme")));
      ...
      CheckResult result =
          zanzibar.check("doc", "readme", "allow_to_update", entry.getKey(), new LocalCache());
      System.out.printf(
          "user %s [expected result: %s] => %s %n",
          entry.getKey(), entry.getValue(), result.isResult());
  ...
  void relationsSimple() {
    Map<String, String> usersToTest =
        Map.of(
            "user1",
            "namespace:object#owner",
            "user2",
            "namespace:object#editor",
            "user3",
            "namespace:object#viewer");
    for (Map.Entry<String, String> entry : usersToTest.entrySet()) {
      ...
      Set<String> result =
          zanzibar.getRelations("namespace", "object", principal, new LocalCache());
      System.out.printf("user %s => %s %n", principal, result);
  ...
  void relationsContacts() {
    ...
    for (Map.Entry<String, String> entry : usersToTest.entrySet()) {
      ...
      Mockito.doReturn(aclsGroupUser1)
          .when(aclService)
          .findAllByNamespaceAndObjectAndUser(eq("group"), eq("user1"), eq(principal));
      Mockito.doReturn(aclsGroupUser2)
          .when(aclService)
          .findAllByNamespaceAndObjectAndUser(eq("group"), eq("user2"), eq(principal));
  ...
  void checkContactUsersTest() {
    ...
    for (Map.Entry<String, Boolean> entry : usersToTest.entrySet()) {
      ...
      Set<Acl> aclsGroupContactusers =
          acls.stream()
              .filter(
                  acl ->
                      acl.getNamespace().equals("group") && acl.getObject().equals("contactusers"))
              .collect(Collectors.toSet());

      Set<Acl> aclsApiContact =
          acls.stream()
      ...
      Mockito.doReturn(aclsGroupContactusers)
          .when(aclService)
          .findAllByPrincipalAndNsObjectIn(eq(principal), eq(List.of("group:contactusers")));
      Mockito.doReturn(aclsApiContact)
          .when(aclService)
          .findAllByNamespaceAndObjectAndUser(eq("api"), eq("contact"), eq(principal));
      Mockito.doReturn(aclsApiContact)
          .when(aclService)
  ...
  void wildcardRelationsTest() {
    ...
    for (Map.Entry<String, Boolean> entry : usersToTest.entrySet()) {
      ...
      Set<Acl> aclsGroupContactusers =
          new ArrayList<>(acls)
              .stream()
                  .filter(
                      acl ->
                          acl.getNamespace().equals("group")
                              && acl.getObject().equals("contactusers")
                              && principal.equals(acl.getUser()))
                  .collect(Collectors.toSet());
      ...
      Mockito.doReturn(aclsGroupContactusers)
          .when(aclService)
          .findAllByPrincipalAndNsObjectIn(eq(principal), eq(List.of("group:contactusers")));
      Mockito.doReturn(aclsContact)
          .when(aclService)
          .findAllByNamespaceAndObjectAndUser(eq("contact"), eq(uuid), eq(principal));
      Mockito.doReturn(aclsContact)
          .when(aclService)
          .findAllByPrincipalAndNsObjectIn(
              eq(principal), eq(List.of(String.format("contact:%s", uuid))));

      System.out.println("user: " + principal);
      // user1 and user2 are have access. And user3 is not.
      assertEquals(
          expected,
          zanzibar.check("contact", uuid, "viewer", principal, new LocalCache()).isResult());
    }
  }
  ...
  void exclusionTest() {
    ...
    relationConfigContact.setRelations(
        Set.of(
            AclRelation.builder()
                .object("object")
                .relation("rel1")
                .exclusions(Set.of("rel2"))
                .build(),
            AclRelation.builder()
                .object("object")
                .relation("rel2")
                .exclusions(Set.of("rel1"))
                .build()));
    ...
    for (Map.Entry<String, Boolean> entry : usersToTest.entrySet()) {
      ...
      Mockito.doReturn(acls)
          .when(aclService)
          .findAllByPrincipalAndNsObjectIn(anyString(), eq(List.of("group:object")));
      ...
      assertEquals(
          expected,
          zanzibar.check("group", "object", "rel1", principal, new LocalCache()).isResult());
    }
  }
    ...
    relationConfigContact.setRelations(
        Set.of(
            AclRelation.builder()
                .object("object")
                .relation("rel1")
                .intersections(Set.of("rel2"))
                .build(),
            AclRelation.builder()
                .object("object")
                .relation("rel2")
                .intersections(Set.of("rel1"))
                .build()));
  ...
  private AclRelationConfig prepConfig() {
    ...
    relationConfig.setRelations(
        Set.of(
            AclRelation.builder().object("document").relation("owner").build(),
            AclRelation.builder()
                .object("document")
                .relation("editor")
                .parents(Set.of(AclRelationParent.builder().relation("owner").build()))
  ...
  private List<AclRelationConfig> prepConfig2() {
    ...
    relationConfig.setRelations(
        Set.of(
            AclRelation.builder().object("user1").relation("owner").build(),
            AclRelation.builder()
                .object("user1")
                .relation("editor")
                .parents(Set.of(AclRelationParent.builder().relation("owner").build()))
    ...
    relationConfig2.setRelations(
        Set.of(
            AclRelation.builder().object("user2").relation("owner").build(),
            AclRelation.builder()
                .object("user2")
                .relation("editor")
                .parents(Set.of(AclRelationParent.builder().relation("owner").build()))
