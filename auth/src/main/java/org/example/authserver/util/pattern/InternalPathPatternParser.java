...
class InternalPathPatternParser {
   ...
   *
   * <p>Nested {...} pairs don't have to be escaped: <code>/abc/{var:x{1,2}}/def</code>
   *
  ...
  /**
   * After processing a separator, a quick peek whether it is followed by a double wildcard (and
   * only as the last path element).
   */
  private boolean peekDoubleWildcard() {
    ...
    char separator = this.parser.getPathOptions().separator();
    if ((this.pos + 3) < this.pathPatternLength
        && this.pathPatternData[this.pos + 3] == separator) {
      throw new PatternParseException(
          this.pos,
          this.pathPatternData,
          PatternParseException.PatternMessage.NO_MORE_DATA_EXPECTED_AFTER_CAPTURE_THE_REST);
        ...
        if (this.isCaptureTheRestVariable) {
          ...
          try {
            ...
            throw new PatternParseException(
                ...
                this.pathPatternData,
                PatternParseException.PatternMessage.REGEX_PATTERN_SYNTAX_EXCEPTION);
          }
